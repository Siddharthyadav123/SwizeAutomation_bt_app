...
import com.svizeautomation.app.util.AssetUtils;
...
/**
 * This Activity appears as a dialog. It lists already paired devices, and it
 * can scan for devices nearby. When the user selects a device, its MAC address
 * is returned to the caller as the result of this activity.
 */
public class DeviceListActivity extends Activity {
    ...
    public static enum Message {
        DeviceConnectorType, BluetoothAddress, MockFilename,
    }
    ...
    private final BluetoothAdapterWrapper mBtAdapter = BluetoothAdapterFactory
            .getBluetoothAdapterWrapper();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().getBooleanExtra(EXTRA_MOCK_DEVICES_ENABLED, false)) {
            String[] filenames = AssetUtils.listFiles(getResources()
                    .getAssets(), MockLineByLineConnector.SAMPLES_SUBDIR);
            if (filenames.length > 0) {
                ArrayAdapter<MockDeviceEntry> mockDevicesAdapter = new ArrayAdapter<MockDeviceEntry>(
                ...
                mockListView
                        .setOnItemClickListener(new MockDeviceClickListener(
                                mockDevicesAdapter));
                ...
                findViewById(R.id.title_mock_devices).setVisibility(
                        View.VISIBLE);
        ...
        if (pairedDevices != null && !pairedDevices.isEmpty()) {
            ArrayAdapter<BluetoothDeviceEntry> pairedDevicesAdapter = new ArrayAdapter<BluetoothDeviceEntry>(
                    this, R.layout.device_name);
            ListView pairedListView = (ListView) findViewById(R.id.paired_devices);
            pairedListView.setAdapter(pairedDevicesAdapter);
            pairedListView
                    .setOnItemClickListener(new BluetoothDeviceClickListener(
                            pairedDevicesAdapter));

            for (BluetoothDevice device : pairedDevices) {
                pairedDevicesAdapter.add(new BluetoothDeviceEntry(device
        ...
        mNewDevicesArrayAdapter = new ArrayAdapter<BluetoothDeviceEntry>(this,
        ...
        IntentFilter bluetoothDeviceFoundFilter = new IntentFilter(
    ...
    private class BluetoothDeviceClickListener implements OnItemClickListener {
        ...
        private BluetoothDeviceClickListener(
                ArrayAdapter<BluetoothDeviceEntry> adapter) {
        ...
        public void onItemClick(AdapterView<?> adapterView, View view, int i,
            ...
            intent.putExtra(Message.DeviceConnectorType.toString(),
                    ConnectorType.Bluetooth);
            intent.putExtra(Message.BluetoothAddress.toString(),
                    adapter.getItem(i).address);
    ...
    private class MockDeviceClickListener implements OnItemClickListener {
        ...
        public void onItemClick(AdapterView<?> av, View v, int arg2, long arg3) {
            ...
            intent.putExtra(Message.DeviceConnectorType.toString(),
                    ConnectorType.Mock);
            intent.putExtra(Message.MockFilename.toString(),
                    adapter.getItem(arg2).filename);
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothDevice.ACTION_FOUND.equals(action)) {
                ...
                BluetoothDevice device = intent
                        .getParcelableExtra(parcelableExtraName);
                if (device != null) {
                    String address = device.getAddress();
                    if (!mNewDevicesSet.contains(address)) {
                        mNewDevicesSet.add(address);
                        mNewDevicesArrayAdapter.add(new BluetoothDeviceEntry(
                                device.getName(), address));
                    }
                } else {
                    Log.e(TAG, "Could not get parcelable extra: "
                            + parcelableExtraName);
                }
                ...
                if (mNewDevicesSet.isEmpty()) {
                    findViewById(R.id.label_none_found).setVisibility(
                            View.VISIBLE);
                } else {
                    findViewById(R.id.label_none_found)
