 ...
 */
@SuppressLint("NewApi") public class BottomSheet extends Dialog implements DialogInterface {
    ...
    public static class Builder {
        ...
        public Builder sheet(List<Sheet> itemList) {
        	parseList(itemList);
            return this;
        }

		private void parseList(List<Sheet> itemList) {
			try {
				if (itemList != null) {					
					for (int i = 0; i < itemList.size(); i++) {
						MenuItem item = new MenuItem();
						item.id = i;
						item.text = itemList.get(i).getText();
						if (itemList.get(i).getIconId() > 0)
                            item.icon = context.getResources().getDrawable(itemList.get(i).getIconId());
						menuItems.add(item);
					}
